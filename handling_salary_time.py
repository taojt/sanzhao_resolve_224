...
def handle_work_year(work_year):
	...
	if u"暂时没有工作经验" in work_year or u"应届" in work_year or u"实习" in work_year or u"毕业生" in work_year:
		return "0"
	if re.match(u"(\d+)\s*年(\d+)\s*个?月", work_year):
		temp = re.match(u"(\d+)\s*年(\d+)\s*个?月", work_year)
		s = str(int(temp.group(1)) * 12 + int(temp.group(2)))
		return s
	elif u"以上工作经验" in work_year:
		temp = re.match(u"(\d+)年以上工作经验", work_year)
		if temp:
		...
		return s
	elif re.match(u"^\d{1,2}$", work_year):
		s = str(int(int(work_year) *12))
		return s
...
def handle_salary(salary):
	...
	if re.match(u"(\d+-\d+)元?/月", salary):
		return re.match(u"(\d+-\d+)元?/月", salary).group(1).strip()

	return ""

def num_to_str(num):
	"""
	numberLong 时间戳转字符串时间
	:param num:
	:return:
	"""
	try:
		ltime = time.localtime(int(num))
		timeStr = time.strftime("%Y-%m", ltime)
		return timeStr
	except:
		return ""