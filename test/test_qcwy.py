...
from handling_resume.handling_qcwy import handling_qcwy
from convert_time import *
from mongo_connect import *
import logging
import constant

log = logging.getLogger("QcwyThread")


def test_qcwy(min_time, max_time):
	"""
	导入解析智联简历方法
	:param min_time:
	:param max_time:
	:return:
	"""
	src_conn = connect(database=constant.db_qcwy, collection=constant.db_qcwy_collection, host=constant.db_src_ip,
	...
	try:
		...
		log.info(u"当前时间段内查找到的前程无忧简历条数是：" + str(count))
	# 捕获mongo 连接超时 异常
	except pymongo.errors.ServerSelectionTimeoutError, e:
		log.error(u"统计前程无忧简历条数错误，数据库连接超时， 异常信息： %s" % e.message)
	except:
		log.error(u"统计前程无忧简历条数错误")
	...
	while True:
		try:
			...
			if src_conn is None:
				raise pymongo.errors.ServerSelectionTimeoutError
			# 前程无忧简历通过爬取时间"last_crawled_time" 来导入
		...
		for d in db_cursor:
			...
			# 插入数据时 数据库连接超时
				...
				if isinstance(resume, dict) and resume.has_key("cv_id"):
					log.error(u"前程无忧数据解析异常， 异常数据Cv_id：%s" % resume["cv_id"])
				else:
					log.error(u"前程无忧数据解析异常,解析过程中出现错误!")

			temp += 1
			all_temp += 1
		end = datetime.datetime.now()
		log.info(u"前程无忧完成导入 %s 条数据，一共耗时 %d 秒 ！" % (all_temp, (end - start).seconds))
		# 判断是否是已经读取完数据，因为是分页查询的，每页1000条数据，若不满1000条则是最后的数据
		if temp % page_size != 0 or temp == 0:
			log.info(u"---------- 前程无忧已经导入所有数据")
			break
