...
from handling_resume.handling_zhilian import handling_zhilian
from convert_time import *
from mongo_connect import *
import logging
import constant

log = logging.getLogger("ZhilianThread")


def test_zhilian(min_time, max_time):
	"""
	导入解析智联简历方法
	:param min_time:
	:param max_time:
	:return:
	"""
	src_conn = connect(database=constant.db_zhilian, collection=constant.db_zhilian_collection, host=constant.db_src_ip,
	...
	try:
		...
		# 统计当前时间段内简历条数：
		count = src_conn.count({"Crawled_timestamp": {"$gte": min_time, "$lt": max_time}})
		log.info(u"当前时间段内查找到的智联简历条数是：" + str(count))
	# 捕获mongo 连接超时 异常
	except pymongo.errors.ServerSelectionTimeoutError, e:
		log.error(u"统计智联简历条数错误，数据库连接超时， 异常信息： %s" % e.message)
	except:
		log.error(u"统计智联简历条数错误")
	...
	while True:
		try:
			...
			if src_conn is None:
				raise pymongo.errors.ServerSelectionTimeoutError
			# 智联简历通过爬取时间"Crawled_timestamp" 来导入
			db_cursor = src_conn.find({"Crawled_timestamp": {"$gte": min_time, "$lt": max_time}}).skip(
		...
		# 捕获mongo 连接超时 异常
			...
			time.sleep(3590)
			# 重新建立到数据库的连接
			src_conn = connect(database=constant.db_zhilian, collection=constant.db_zhilian_collection,
		...
		for d in db_cursor:
			...
			# 插入数据时 数据库连接超时
				...
				if isinstance(resume, dict) and resume.has_key("cv_id"):
					log.error(u"智联数据解析异常， 异常数据Cv_id：%s" % resume["cv_id"])
				else:
					log.error(u"智联数据解析异常,解析过程中出现错误!")

			temp += 1
			all_temp += 1
		end = datetime.datetime.now()
		log.info(u"智联完成导入 %s 条数据，一共耗时 %d 秒 ！" % (all_temp, (end - start).seconds))
		# 判断是否是已经读取完数据，因为是分页查询的，每页1000条数据，若不满1000条则是最后的数据
		if temp % page_size != 0 or temp == 0:
			log.info(u"---------- 智联已经导入所有数据")
			break
